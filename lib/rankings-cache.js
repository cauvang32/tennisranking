...
export default class RankingsCache {
  constructor({ ttl = 5 * 60 * 1000, devLogging = false } = {}) {
    ...
    // Cache version for client synchronization
    // Increments on any data change, clients use this to invalidate stale cache
    ...
    // HMAC secret for cache integrity (regenerated on restart)
    this.integritySecret = crypto.randomBytes(32).toString('hex')
  ...
  // Generate HMAC for cache entry integrity
  generateIntegrity(key, data) {
    const hmac = crypto.createHmac('sha256', this.integritySecret)
    hmac.update(key)
    hmac.update(JSON.stringify(data))
    return hmac.digest('hex')
  }

  // Verify HMAC integrity of cache entry
  verifyIntegrity(key, data, storedHmac) {
    const expectedHmac = this.generateIntegrity(key, data)
    try {
  ...
  set(key, data, ttl = this.defaultTTL) {
    ...
    const hmac = this.generateIntegrity(key, data)
    this.cache.set(key, {
      data,
      hmac,
      sizeBytes: newSize,
  ...
  get(key) {
    ...
    if (item.hmac && !this.verifyIntegrity(key, item.data, item.hmac)) {
      this.currentMemoryBytes -= item.sizeBytes || 0
      this.cache.delete(key)
      this.stats.integrityFailures++
      this.stats.misses++
      this.log(`🔒 Cache INTEGRITY FAILURE: ${key} - possible tampering detected`)
      return null
    }
  ...
  invalidate(pattern = null) {
    ...
    // Increment version so clients know to refresh
    this.dataVersion = Date.now()
  }
