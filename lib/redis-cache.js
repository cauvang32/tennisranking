...
export default class RedisCache extends EventEmitter {
   ...
   * Clears: rankings, players list, season player lists, matches, playdates
   */
  async invalidateOnPlayerChange() {
    ...
    await Promise.all([
      ...
      this.invalidateByPrefix('matches:'), // Player deletion cascades to matches
      this.delete('playdates'),
      this.delete('playdate:latest')
    ])
   ...
   * Clears: rankings, seasons lists, active season, season player lists, matches
   */
  async invalidateOnSeasonChange() {
    ...
    await Promise.all([
      ...
      this.invalidateByPrefix('season:'),  // Clears season:active AND season:*:players
      this.invalidateByPrefix('matches:')  // Season name/lose_money affects match display
    ])
  ...
  async preloadCommonData(db, force = false) {
    ...
    try {
      ...
      try {
        // Reuse playDates from step 5 cache or fetch if not available
        let cachedPlayDates = await this.get('playdates')
        if (!cachedPlayDates) cachedPlayDates = await db.getPlayDates()
        const recentDates = (cachedPlayDates || []).slice(0, 5).map(d => d.play_date?.split('T')[0] || d.play_date)
